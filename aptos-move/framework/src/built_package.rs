...
impl BuiltPackage {
    ...
    pub fn extract_metadata(&self) -> anyhow::Result<PackageMetadata> {
        ...
        for u in self.package.root_modules() {
            ...
            let source = if self.options.with_srcs {
                zip_metadata_str(&std::fs::read_to_string(&u.source_path).with_context(|| {
                    format!("Failed to read source file at {:?}", u.source_path)
                })?)?
            } else {
                vec![]
            };
    ...
    pub fn extract_metadata_and_save(&self) -> anyhow::Result<()> {
        ...
        std::fs::write(path.join(METADATA_FILE_NAME), bcs::to_bytes(&data)?).with_context(
            || {
                format!(
                    ...
                    if !module_metadata.is_empty() {
                        ...
                        if path.is_file() {
                            ...
                            std::fs::write(&path, bytes).with_context(|| {
                                format!("Failed to write compiled module to {:?}", path)
                            })?;
                        }
                    }
                }
